...
MAX_BATCH_SIZE = 5  # Maximum number of files to be processed in a batch
...
def save_files(message, max_message_length=4000):
    ...
    # Calculate the number of files remaining for the batch
    ...
    for file_name in updated_file_names:
        ...
        if len("".join(updated_files_list)) + len(file_name_html) > max_message_length:
            ...
            bot.send_message(
                message.chat.id, "".join(updated_files_list), parse_mode="HTML"
            )
            updated_files_list = []
...
def handle_log_command(message):
    ...
    random_number = random.uniform(15.25, 15.3)
...
def custom_sort(x):
    keywords_present = any(
        keyword.lower() in x[0].lower() for keyword in ["x265", "10bit", "mkv", "mp4"]
    )
...
def handle_search(message):
    ...
    if num_results == 0:
        try:
            ...
            file_name_scores = [
                (file_name, similarity_score(search_query, normalize_string(file_name)))
            ...
            message_text = (
                ...
                "*Bay*\n*Baymax*\n*Baymax S01*\n*Baymax S01E02*\n\n"
                "Meanwhile, here are some files closely related to "
                f'"{search_query.title()}" that are available in our database\t💁  :'
            )
            ...
            for suggestion in top_suggestions:
                keyboard.add(
                    telebot.types.InlineKeyboardButton(
                        suggestion,
                        callback_data="suggestion_" + suggestion,
                    )
                )
